...
// Aceeasi cheie configurabila ca in server.js (foloseste-o pe amandoua daca ai o
// cheie personala de la RegieLive, ca sa fii independent de bugetul cheii comune).
...
// Cand o metoda de cautare (Nume+An sau Nume) pica cu eroare/404 (rate-limit sau hiccup
// trecator la RegieLive), rezultatul GOL era cachet tot 5 minute - inghetand lipsa unor
// potriviri bune gasite cu putin timp inainte, pt. toate cererile din urmatoarele 5 minute.
// Un esec de API nu inseamna "acest film n-are subtitrari" (caz in care 5 minute e ok),
// asa ca il cachem mult mai scurt - suficient cat un retry normal sa recupereze rezultatele.
...
// Fara acest cap, o cautare poate ramane agatata minute intregi daca bugetul e epuizat
// (ex: teste repetate) - mai bine abandonam grabnic acea metoda de cautare si continuam
// cu urmatoarea (fiecare metoda din _searchRegieLive isi prinde deja propria eroare).
...
async function searchRegieLive(imdbId, type, videoFilename) {
    ...
    try {
        ...
        const ttl = errorTracker.hadApiError ? ERROR_RESULT_CACHE_TTL_MS : SEARCH_CACHE_TTL_MS;
        searchResultCache.set(cacheKey, { data: result, expiresAt: Date.now() + ttl });
        return result;
    } finally {
    ...
    function ingestResponse(response, { isFallbackByName, referenceTitle }) {
        ...
        for (const filmKey in filme) {
            ...
            for (const subKey in subs) {
                ...
                const subUrl = subs[subKey].url || '';
                // RegieLive are ocazional inregistrari corupte, cu link de forma
                // ".../descarca-XXXXX-0.zip" -> "Link invalid / Nu exista filmul
                // si/sau subtitrarea." Le excludem din start, ca sa nu ajunga
                // sa fie alese automat si sa dea "subtitle failed to load".
                if (/-0\.zip$/i.test(subUrl)) {
                    console.log(`[FILTRU LINK] Ignor subtitrare cu link invalid (ID 0): "${subs[subKey].titlu}"`);
                    continue;
                }
