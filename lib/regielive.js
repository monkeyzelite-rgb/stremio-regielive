...
async function searchRegieLive(imdbId, type) {
    ...
    // 1. Adăugăm oricum ID-ul IMDb (API-ul Bazarr îl prioritizează de obicei pentru precizie 100%)
    ...
    // 2. Extragem ANUL mult mai robust (din year sau releaseInfo, luând doar primele 4 caractere)
    ...
    // Afișăm în consolă ce trimitem exact către RegieLive
    console.log(`[DEBUG API] Parametri trimiși către RegieLive:`, params);
    ...
    try {
        const response = await axios.get(API_URL, {
            ...
            headers: { 
                'RL-API': API_KEY,
                // 3. User-Agent corect pentru a nu fi confundați cu boți de spam
